...
import { ProcessSymbol } from '@/components/symbols';
    ...
    >
      ...
      <div className="mb-1.5 flex items-center justify-between gap-2">
        <div className="flex min-w-0 flex-col">
          {/* 1차 라벨: equipmentId (PROC-001 등 친숙 코드) */}
          <span className="max-w-[110px] truncate text-[10px] font-semibold text-gray-700">
            {data.equipmentId ?? data.name}
          </span>
          {/* 2차 라벨: name (equipmentId 와 다를 때만 표시) */}
          {data.name && data.name !== data.equipmentId && (
            <span className="max-w-[110px] truncate text-[8px] text-gray-400">{data.name}</span>
          )}
        </div>
      ...
      {/* Process 심볼 */}
      <ProcessSymbol state={data.state} width={120} height={48} className="block" />
...
ProcessGroupNode.displayName = 'ProcessGroupNode';
